...
class TestSecurityGroupsBasicOps(manager.NetworkScenarioTest):
    ...
    def _check_remote_connectivity(self, source, dest, should_succeed=True):
        ...
        def ping_remote():
            try:
                ...
                #cmd = 'curl --max-time 2 %s:22 2> /dev/null > output; cat output' % dest
                result = source.exec_command(cmd)
                LOG.warning("NC result: %s, type: %s" % (result, type(result)))
                #if result[:3] != "SSH":
                if result != "0":
    ...
    def _test_cross_tenant_allow(self, source_tenant, dest_tenant):
        ...
        self._create_sg_ssh_rule(source_tenant)
    ...
    def test_cross_tenant_traffic(self):
        try:
            ...
            for tenant in self.tenants.values():
                pass #self._log_console_output(servers=tenant.servers)
            raise
